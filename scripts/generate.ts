...
export class Context {
  ...
  public USE_PRIMJS = process.env.QTS_USE_PRIMJS === "true" || process.env.LEPUS_VARIANT === "1"
}
...
const DECL_RE = /^([\w()* ]+[\s*]+)((?:QTS_|LEPUS_)\w+)(\((.*?)\)) ?{$/gm
...
function main() {
  ...
  if (command === "lepus-ffi") {
    writeFile(destination, buildLEPUSFFI(context, matches))
    return
  }
...
type Attribute = "MaybeAsync" | "AsyncifyOnly" | "DebugOnly" | "PrimJSOnly"
...
function getAvailableDefinitions(context: Context, matches: RegExpMatchArray[]) {
  ...
  const filtered = parsed.filter((fn) => {
    ...
    if (fn.returnType.attributes.has("PrimJSOnly")) {
      // Only include PrimJS-only functions when building PrimJS variants
      return context.USE_PRIMJS
    }
...
export function buildLEPUSFFI(context: Context, matches: RegExpExecArray[]) {
  const parsed = getAvailableDefinitions(context, matches)
  
  // Filter for LEPUS functions only
  const lepusFunctions = parsed.filter((fn) => fn.functionName.startsWith("LEPUS_"))
  
  const decls: string[] = []
  lepusFunctions.forEach((fn) => {
    if (!fn.returnType.attributes.has("AsyncifyOnly") || context.ASYNCIFY) {
      decls.push(renderLEPUSFunction({ ...fn, async: false, context }))
    }

    if (fn.returnType.attributes.has("MaybeAsync") && context.ASYNCIFY) {
      decls.push(renderLEPUSFunction({ ...fn, async: true, context }))
    }
  })

  const { ASYNCIFY, TYPE_ONLY, DEBUG } = context
  const declType = TYPE_ONLY ? "interface" : "class"
  
  // LEPUS-specific types
  const lepusTypes = [
    "LEPUSRuntimePointer",
    "LEPUSContextPointer", 
    "LEPUSValuePointer",
    "LEPUSValueConstPointer",
    "LEPUSModuleDefPointer",
    "LEPUSEmscriptenModule"
  ]
 ...
 * @unstable The FFI interface is considered private and may change.
 */
export ${declType} LEPUSFFI {
  ${TYPE_ONLY ? "" : `constructor(public module: LEPUSEmscriptenModule) {}`}
  /** Set at compile time. */
  ${TYPE_ONLY ? "readonly DEBUG: boolean" : `readonly DEBUG = ${DEBUG}`}
...
function renderLEPUSFunction(args: {
  context: Context
  functionName: string
  returnType: ParsedType
  params: Array<{ name: string; type: ParsedType }>
  ...
  // Map C types to LEPUS TypeScript types
  const mapToLepusType = (tsType: string): string => {
    switch (tsType) {
      case "JSRuntimePointer": return "LEPUSRuntimePointer"
      case "JSContextPointer": return "LEPUSContextPointer"
      case "JSValuePointer": return "LEPUSValuePointer"
      case "JSValueConstPointer": return "LEPUSValueConstPointer"
      case "JSModuleDefPointer": return "LEPUSModuleDefPointer"
  ...
  const typescriptParams = params
    .map((param) => {
      const lepusType = mapToLepusType(param.type.typescript)
      const tsType = lepusType === "LEPUSValueConstPointer"
        ? "LEPUSValuePointer | LEPUSValueConstPointer"
        : lepusType
      return `${param.name}: ${tsType}`
    })
    .join(", ")
  ...
  const lepusReturnType = mapToLepusType(returnType.typescript)
  const typescriptReturnType = async && returnType.attributes.has("MaybeAsync")
    ? `${lepusReturnType} | Promise<${lepusReturnType}>`
    : lepusReturnType
