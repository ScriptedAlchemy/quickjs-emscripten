...
async function testAsyncFSWorkerSimulation() {
    ...
    // Initialize LEPUS directly
    const lepus = await createLEPUSModule();
    
    // For this test, we'll create a simple wrapper to maintain compatibility
    const vm = {
        newObject: () => ({}),
        newFunction: (name, fn) => fn,
        setProp: (obj, key, val) => { obj[key] = val; },
        evalCode: (code) => {
            try {
                return { value: lepus.evalCode(code) };
            } catch (error) {
                return { error };
            }
        },
        dispose: () => lepus.dispose()
    };
