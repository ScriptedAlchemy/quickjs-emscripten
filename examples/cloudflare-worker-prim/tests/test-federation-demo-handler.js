...
(async () => {
  ...
  if (response.status === 200) {
    ...
    if (!responseData.message || !responseData.message.includes('Module Federation Demo')) {
    ...
    if (responseData.execution_endpoint !== '/execute-module') {
      throw new Error(`❌ Wrong execution endpoint: ${responseData.execution_endpoint}`);
    }
    ...
    console.log('📋 Execution endpoint:', responseData.execution_endpoint);
  ...
  const hasExecutionEndpoint = infoData.execution_endpoint && infoData.execution_endpoint.length > 0;
